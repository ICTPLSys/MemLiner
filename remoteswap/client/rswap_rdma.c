											   ...
											   unsigned int cpu, enum rdma_queue_type type)
{
	...
	if (type < NUM_QP_TYPE)
	{
		...
		{
			print_debug(KERN_ERR "%s, rdma queue index overflow\n", __func__);
			return NULL;
		}
	}
	else
...
inline void get_rdma_queue_cpu_type(struct rdma_session_context *rdma_session,
									struct rswap_rdma_queue *rdma_queue, unsigned int *cpu, enum rdma_queue_type *type)
{
...
int handle_recv_wr(struct rswap_rdma_queue *rdma_queue, struct ib_wc *wc)
{
	...
	{ // Check the length of received message
	...
	case FREE_SIZE:
		...
		rdma_session->remote_mem_pool.chunk_num = rdma_session->rdma_recv_req.recv_buf->mapped_chunk;
 ...
 * Used for RDMA connection build.
 *
 */
int send_message_to_remote(struct rdma_session_context *rdma_session, int rdma_queue_ind, int message_type, int chunk_num)
{
	int ret = 0;
	...
	{
		printk(KERN_ERR
			   "%s, Post 2-sided message to receive data failed.\n",
			   __func__);
		goto err;
	}
	...
	print_debug(
		"Send a Message to memory server. send_buf->type : %d, %s \n",
...
int rswap_request_for_chunk(struct rdma_session_context *rdma_session)
{
	...
	ret = send_message_to_remote(rdma_session, 0, REQUEST_CHUNKS, chunk_num);
	if (ret)
	{
		printk(KERN_ERR "%s, Post 2-sided message to remote server failed.\n", __func__);
		goto err;
...
int rswap_create_rdma_queue(struct rdma_session_context *rdma_session, int rdma_queue_index)
{
	...
	// 2) Build CQ
	cq_num_cqes = rdma_session->send_queue_depth + rdma_session->recv_queue_depth;
...
void rswap_setup_message_wr(struct rdma_session_context *rdma_session)
{
	...
	rdma_session->rdma_recv_req.recv_sgl.lkey = rdma_session->rdma_dev->pd->local_dma_lkey;
	...
	rdma_session->rdma_send_req.send_sgl.lkey = rdma_session->rdma_dev->pd->local_dma_lkey;
...
int init_remote_chunk_list(struct rdma_session_context *rdma_session)
{
	...
	rdma_session->remote_mem_pool.chunks = (struct remote_chunk *)kzalloc(
		sizeof(struct remote_chunk) * rdma_session->remote_mem_pool.chunk_num,
		GFP_KERNEL);
 ...
 * 		remote_addr : The actual virtual address of the mapped chunk
 *
 *	2) Attach the received chunks to the rdma_session_context->chunk_list->chunks[]
 */
void bind_remote_memory_chunks(struct rdma_session_context *rdma_session)
{
	...
	for (i = 0; i < chunk_num; i++)
	{
		if (rdma_session->rdma_recv_req.recv_buf->rkey[i])
		{
			...
			rdma_session->remote_mem_pool.chunks[i].remote_rkey = rdma_session->rdma_recv_req.recv_buf->rkey[i];
			rdma_session->remote_mem_pool.chunks[i].remote_addr = rdma_session->rdma_recv_req.recv_buf->buf[i];
			rdma_session->remote_mem_pool.chunks[i].mapped_size = rdma_session->rdma_recv_req.recv_buf->mapped_size[i];
			rdma_session->remote_mem_pool.chunks[i].chunk_state = MAPPED;
...
int init_rdma_sessions(struct rdma_session_context *rdma_session)
{
	...
	// After transffer to big endian, the decimal value is 47140
	rdma_session->port = htons(server_port);
	ret = in4_pton(server_ip, strlen(server_ip), rdma_session->addr, -1,
				   NULL); // char* to ipv4 address ?
	if (ret == 0)
	{ // success 1; failed 0.
		printk(KERN_ERR
			   "Assign ip %s to  rdma_session->addr : %s failed.\n",
