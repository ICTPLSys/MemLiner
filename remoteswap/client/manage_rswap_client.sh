...
function create_swap_file () {
	...
	then
		...
		cur_size_Byte=$(du -s ${swap_file} | awk '{print $1;}' | tr -cd '[[:digit:]]')
		cur_size_GB=$(($cur_size_Byte / 1024 / 1024))
		SWAP_PARTITION_SIZE_Byte=$(($SWAP_PARTITION_SIZE_GB * 1024 * 1024))
		if (( "${cur_size_Byte}" < "${SWAP_PARTITION_SIZE_Byte}" ))
		then
			echo "Current ${swap_file}: ${cur_size_GB}G NOT equal to expected ${SWAP_PARTITION_SIZE_GB}G"
			echo "Delete it"
			sudo rm ${swap_file}
			...
			echo "Existing swapfile ${swap_file} , ${cur_size_GB}GB is equal or larger than we want, ${SWAP_PARTITION_SIZE_GB}GB. Reuse it."
		fi
	else
