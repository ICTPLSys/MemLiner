...
void fs_rdma_write_done(struct ib_cq *cq, struct ib_wc *wc)
{
	...
	atomic_dec(&rdma_queue->rdma_post_counter); // decrease outstanding rdma
												// request counter
	complete(&rdma_req->done);					// inform caller, write is done. is this
												// necessary for a write?
	kmem_cache_free(rdma_queue->fs_rdma_req_cache,
...
void fs_rdma_read_done(struct ib_cq *cq, struct ib_wc *wc)
{
	...
	SetPageUptodate(
		rdma_req->page); // [?] will be invoked in swap_readpage(). no need
						 // to do it here ?
					 ...
					 enum rdma_queue_type type)
{
	...
	rdma_req->cqe.done =
		type == QP_STORE ? fs_rdma_write_done : fs_rdma_read_done;
...
static struct frontswap_ops rswap_frontswap_ops = {
	...
	.invalidate_page =
		rswap_invalidate_page, // Invoked to clear a page of memory pool?
